...
- [x] **Step 5: 上电前逐点复核（用户完成自行检查；助手照片核对由用户明确豁免，未执行）**
...
默认在助手照片核对通过前不接通9V适配器。如果用户明确要求跳过照片核对，必须记录该项未执行及用户授权，并由用户自行承担接线逐点确认。本次实施采用了该明确豁免。
...
- [x] **Step 1: 先给STM32上电，保持9V断开**

接通STM32、调试器和串口，观察启动日志正常。此时电机电源VM没有供电，电机不得旋转。

- [x] **Step 2: 在软件停止状态接通9V适配器**
